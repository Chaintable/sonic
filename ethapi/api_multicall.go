...
func handleNative(ctx context.Context, state state.StateDB, msg types.Message) ([]byte, int, error) {
	...
	switch method.Name {
	case "name", "symbol":
		res, err := method.Outputs.Pack("FTM")
		if err != nil {
...
func doOneCall(ctx context.Context, b Backend, state state.StateDB, header *evmcore.EvmHeader, arg TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash) (*callResult, error) {
	var err error
	...
	res, err := DoCall(ctx, b, arg, blockNrOrHash, nil, b.RPCEVMTimeout(), b.RPCGasCap())
	if err != nil {
...
func (s *PublicBlockChainAPI) MultiCall(ctx context.Context, args []TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, pfastFail, puseParallel, pdisableCache *bool, overrides *StateOverride) (resp *multiCallResp, err error) {
	// maximum calls check
	...
	if state == nil || err != nil {
		log.Warn("MultiCall: failed to get state", "err", err)
		return nil, err
	}
