...
func (r debankHeaderReader) Header(hash common.Hash, number uint64) *evmcore.EvmHeader {
	if hash != (common.Hash{}) {
...
func (api *DebankAPI) DebankBlock(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (*debankOutPut, error) {
	...
	evmBlock := types.NewBlockWithHeader(evmBlockHeader).WithBody(types.Body{
	...
	parentStateDB, _, err := api.b.StateAndBlockByNumberOrHash(ctx, rpc.BlockNumberOrHashWithNumber(rpc.BlockNumber(parent.NumberU64())))
	if err != nil {
		return nil, err
	}
	...
	var usedGas uint64
	summary := evmcore.NewStateProcessorForReplay(
		chainConfig,
	...
	if err != nil {
		return nil, fmt.Errorf("failed to read post-state for block %d: %w", block.NumberU64(), err)
	}
	if postState == nil {
		return nil, fmt.Errorf("post-state for block %d not found", block.NumberU64())
	}
